...
public class Board implements Serializable{
	...
	public ArrayList<citiesWithDistancesObj> currentMap = new ArrayList<citiesWithDistancesObj>();
	...
	// Players actions
		public Board simulation(mctsNode n) {
		...
		if(n.visited==0) {
			//remove from unvisited children
			n.getParent().removeUnvisitedChild(n);
			if(n.getParent().getUnvisitedChildren().isEmpty())
				n.getParent().fullyExpanded=true;
			//add this child to the visited list
			n.visited++;
			n.getParent().addVisitedChild(n);
		}
		//update visitCount
		n.visitCount++;
		
		//simulation policy,simulate 1 round ahead
		//players position 0-->1-->2-->3-->0-->1-->2...
		if(n.playerID==0) {
			System.out.println("Starting simulation for player0");
			if(n.level==1) {//this means we have 3 more moves to make
				for(int i=0;i<3;i++) {
					moves = Client.getLegalMoves(n.playerID, newBoard);
					potMove = checkForCureMove(moves,newBoard,n.playerID);
					if(potMove.equals("notCureMove")) {
						if(newBoard.getRoleOf(n.playerID).equals("Quarantine Specialist"))
							potMove = quarantineSpecMove(newBoard,moves,n.playerID);
						else if(newBoard.getRoleOf(n.playerID).equals("Operations Expert"))
							potMove = opsExpertMove(moves,newBoard,n.playerID);
						if(potMove.equals("notAMove") || potMove.equals("notCureMove"))
							potMove = getPromisingTDMoves(newBoard,moves,n.playerID);
					}
					//if(potMove.equals("notAMove"))	
					//	potMove = pickRandomMove(moves);
					newBoard = Client.copyBoard(performMove(potMove,newBoard));
					moves.clear();
				}
				
			}else if (n.level==2) {//this means we have 2 more moves to make
				for(int i=0;i<2;i++) {
					moves = Client.getLegalMoves(n.playerID, newBoard);
					potMove = checkForCureMove(moves,newBoard,n.playerID);
					if(potMove.equals("notCureMove")) {
			...
			}else if(n.level==3) {//this means we have 1 more move to make
				moves = Client.getLegalMoves(n.playerID, newBoard);
				potMove = checkForCureMove(moves,newBoard,n.playerID);
				if(potMove.equals("notCureMove")) {
					if(newBoard.getRoleOf(n.playerID).equals("Quarantine Specialist"))
			...
			//when simulating a level4 node
			//we have no remaining moves,so we just simulate
			//the others to update the stats in the current node
			
			//simulate 4 moves for player1
			for(int i=0;i<4;i++) {
				moves = Client.getLegalMoves(n.playerID+1, newBoard);
				potMove = checkForCureMove(moves,newBoard,n.playerID+1);
				if(potMove.equals("notCureMove")) {
					if(newBoard.getRoleOf(n.playerID+1).equals("Quarantine Specialist"))
						potMove = quarantineSpecMove(newBoard,moves,n.playerID+1);
					else if(newBoard.getRoleOf(n.playerID+1).equals("Operations Expert"))
						potMove = opsExpertMove(moves,newBoard,n.playerID+1);
					if(potMove.equals("notAMove") || potMove.equals("notCureMove"))
						potMove = getPromisingTDMoves(newBoard,moves,n.playerID+1);
				}
				//if(potMove.equals("notAMove"))	
				//	potMove = pickRandomMove(moves);
				newBoard = Client.copyBoard(performMove(potMove,newBoard));
			...
			//simulate 4 moves for player2
			for(int i=0;i<4;i++) {
				moves = Client.getLegalMoves(n.playerID+2, newBoard);
				potMove = checkForCureMove(moves,newBoard,n.playerID+2);
				if(potMove.equals("notCureMove")) {
					if(newBoard.getRoleOf(n.playerID+2).equals("Quarantine Specialist"))
						potMove = quarantineSpecMove(newBoard,moves,n.playerID+2);
					else if(newBoard.getRoleOf(n.playerID+2).equals("Operations Expert"))
						potMove = opsExpertMove(moves,newBoard,n.playerID+2);
					if(potMove.equals("notAMove") || potMove.equals("notCureMove"))
						potMove = getPromisingTDMoves(newBoard,moves,n.playerID+2);
				}
				//if(potMove.equals("notAMove"))	
				//	potMove = pickRandomMove(moves);
				newBoard = Client.copyBoard(performMove(potMove,newBoard));
			...
			//simulate 4 moves for player3
			for(int i=0;i<4;i++) {
				moves = Client.getLegalMoves(n.playerID+3, newBoard);
				potMove = checkForCureMove(moves,newBoard,n.playerID+3);
				if(potMove.equals("notCureMove")) {
					if(newBoard.getRoleOf(n.playerID+3).equals("Quarantine Specialist"))
						potMove = quarantineSpecMove(newBoard,moves,n.playerID+3);
					else if(newBoard.getRoleOf(n.playerID+3).equals("Operations Expert"))
						potMove = opsExpertMove(moves,newBoard,n.playerID+3);
					if(potMove.equals("notAMove") || potMove.equals("notCureMove"))
						potMove = getPromisingTDMoves(newBoard,moves,n.playerID+3);
				}
				//if(potMove.equals("notAMove"))	
				//	potMove = pickRandomMove(moves);
				newBoard = Client.copyBoard(performMove(potMove,newBoard));
			...
			//we simulate for player1
			System.out.println("Starting simulation for player1");
			if(n.level==1) {//this means we have 3 more moves to make
				for(int i=0;i<3;i++) {
					moves = Client.getLegalMoves(n.playerID, newBoard);
					potMove = checkForCureMove(moves,newBoard,n.playerID);
			...
			//we simulate for player2
			System.out.println("Starting simulation for player2");
			if(n.level==1) {//this means we have 3 more moves to make
				for(int i=0;i<3;i++) {
					moves = Client.getLegalMoves(n.playerID, newBoard);
					potMove = checkForCureMove(moves,newBoard,n.playerID);
			...
			//we simulate for player3
			System.out.println("Starting simulation for player3");
			if(n.level==1) {//this means we have 3 more moves to make
				for(int i=0;i<3;i++) {
					moves = Client.getLegalMoves(n.playerID, newBoard);
					potMove = checkForCureMove(moves,newBoard,n.playerID);
	...
	public String pickRandomMove(ArrayList<String> r) {
		...
		return r.get(rn.nextInt(upb-1));		
	}
	
	public String findMinCards(Board b,int pID) {
		int[] cards = {Client.cardsCounterOfColor(b, pID, "Yellow")
					,Client.cardsCounterOfColor(b, pID, "Red")
					,Client.cardsCounterOfColor(b, pID, "Black")
					,Client.cardsCounterOfColor(b, pID, "Blue")};
		int min = Arrays.stream(cards).min().getAsInt();
	...
	public String checkDTmoves(ArrayList<String> m,String toGo,int playerID) {
		
		String toRet = "notAMove";
		
		for(String move: m) {
			if(move.equals("#DT,"+playerID+","+toGo)) {
				toRet=move;
				return toRet;
			}
		}
	...
	public String checkFlyMove(ArrayList<String> m,String toGo,int playerID) {
		String toRet = "notAMove";
		
		
		for(String move: m) {
			if(move.equals("#DF,"+playerID+","+toGo)) {
				toRet = move;
				return toRet;
			}else if(move.equals("#SF,"+playerID+","+toGo)) {
				toRet = move;
				return toRet;
			}else if(move.equals("#CF,"+playerID+","+toGo)) {
				toRet = move;
				return toRet;
		}
	}
	...
	public String checkTDmoves(ArrayList<String>m,String toTreat,int playerID,String color) {
		
		
		for(String move:m) {
			if(move.equals("#TD,"+playerID+","+toTreat+","+color))
				return move;
		}
		return "notAMove";
...
public String getPromisingTDMoves(Board b, ArrayList<String> posMoves,int thePlayerID) {
		
		ArrayList<citiesWithDistancesObj> distanceMap = new ArrayList<citiesWithDistancesObj>();		
		distanceMap = Client.buildDistanceMap(b, b.getPawnsLocations(thePlayerID), distanceMap);
		String  movetoexecute = "notAMove";
		String badCity = "notAMove;";
		String goodCity = "notAMove;";
		City myCity = searchForCity(b.getPawnsLocations(thePlayerID));
		if(b.getCubesLeft(0)<=10) {
			for(City c: getCitiesOfColour("Black")) {
				if(c.getBlackCubes()>=2) {
					movetoexecute = checkTDmoves(posMoves,c.getName(),thePlayerID,"Black");
					if(!movetoexecute.equals("notAMove"))
						return movetoexecute;
					movetoexecute = checkDTmoves(posMoves,c.getName(),thePlayerID);
					if(movetoexecute.equals("notAMove"))
						movetoexecute = checkFlyMove(posMoves,c.getName(),thePlayerID);
					else
						return movetoexecute;
				}
				if(!movetoexecute.equals("notAMove"))
		...
		}else if(b.getCubesLeft(1)<=10) {
			for(City c: getCitiesOfColour("Yellow")) {
				if(c.getYellowCubes()>=2) {
					movetoexecute = checkTDmoves(posMoves,c.getName(),thePlayerID,"Yellow");
					if(!movetoexecute.equals("notAMove"))
						return movetoexecute;
					movetoexecute = checkDTmoves(posMoves,c.getName(),thePlayerID);
					if(movetoexecute.equals("notAMove"))
		...
		}else if(b.getCubesLeft(2)<=10) {
			for(City c: getCitiesOfColour("Blue")) {
				if(c.getBlueCubes()>=2) {
					movetoexecute = checkTDmoves(posMoves,c.getName(),thePlayerID,"Blue");
					if(!movetoexecute.equals("notAMove"))
						return movetoexecute;
					movetoexecute = checkDTmoves(posMoves,c.getName(),thePlayerID);
					if(movetoexecute.equals("notAMove"))
		...
		}else if(b.getCubesLeft(3)<=10) {
			for(City c: getCitiesOfColour("Red")) {
				if(c.getRedCubes()>=2) {
					movetoexecute = checkTDmoves(posMoves,c.getName(),thePlayerID,"Red");
					if(!movetoexecute.equals("notAMove"))
						return movetoexecute;
					movetoexecute = checkDTmoves(posMoves,c.getName(),thePlayerID);
					if(movetoexecute.equals("notAMove"))
			...
			if(myCity.getMaxCube()>= 1) {
				movetoexecute = checkTDmoves(posMoves,myCity.getName(),thePlayerID,myCity.getColour());
				if(!movetoexecute.equals("notAMove"))
					return movetoexecute;
			}
		
			badCity = Client.getMostInfectedInRadius(1, distanceMap, b);
			if(!badCity.equals(myCity.getName())) {
				movetoexecute = checkDTmoves(posMoves,badCity,thePlayerID);
				return movetoexecute;
			}
			for(int i=2;i<7;i++) {
			badCity = Client.getMostInfectedInRadius(i, distanceMap, b);
			if(!badCity.equals(myCity.getName())) {
				goodCity = Client.getDirectionToMove(myCity.getName(), badCity, distanceMap, b);
				movetoexecute = checkDTmoves(posMoves,goodCity,thePlayerID);
			}
			}
	...
	public String getSpecialDirection(String currCity, String targetCity, Board b,int thePlayerID) {
		
		City targetCityObj = searchForCity(targetCity);
		City currCityObj = searchForCity(currCity);
		
		if(cityList.get(cityList.indexOf(targetCityObj)).getHasReseachStation()) { 
			for(int i=0; i<cityList.get(cityList.indexOf(currCityObj)).getNeighboursNumber(); i++) {
				City tempCity = cityList.get(cityList.indexOf(currCityObj));
				City neighboorCity = searchForCity(cityList.get(cityList.indexOf(tempCity)).getNeighbour(i));
				if(neighboorCity.getHasReseachStation()) {
					String toGoCity = cityList.get(cityList.indexOf(currCityObj)).getNeighbour(i);
					return "#SF,"+thePlayerID+","+toGoCity;
				}
			}
		}else if(cityList.get(cityList.indexOf(currCityObj)).getHasReseachStation()){
			for(int i=0; i<cityList.get(cityList.indexOf(targetCityObj)).getNeighboursNumber(); i++) {				
				City tempTargetCity = cityList.get(cityList.indexOf(targetCityObj));
				City neighboorCity2 = searchForCity(cityList.get(cityList.indexOf(tempTargetCity)).getNeighbour(i));
				if(neighboorCity2.getHasReseachStation()) {
					String sfCity = cityList.get(cityList.indexOf(targetCityObj)).getNeighbour(i);
					return "#SF,"+thePlayerID+","+sfCity;
				}
			}		
		}
		return "notAMove";
...
public String quarantineSpecMove(Board b,ArrayList<String> posMoves,int playerID) {
		
		ArrayList<citiesWithDistancesObj> distanceMap = new ArrayList<citiesWithDistancesObj>();		
		distanceMap = Client.buildDistanceMap(b, b.getPawnsLocations(playerID), distanceMap);
		int maxInfectedNeighboursCount = -1;
		int infectedNeighboursCount = 0;
		City targetCity = null;
		String cityToGo;
		String movetoexecute = "notAMove";
		
		for(City city : cityList) {
			if(city.getName()!=b.getPawnsLocations(playerID)){
				if(city.getMaxCube() > 2) {
					for(int i=0; i < city.getNeighboursNumber(); i++) {
						if(b.searchForCity(city.getNeighbour(i)).getMaxCube()>1 && b.searchForCity(city.getNeighbour(i)).getMaxCubeColor()==city.getMaxCubeColor()) {
							infectedNeighboursCount++;
						}
					}
				}
				
			}
			if(infectedNeighboursCount>maxInfectedNeighboursCount) {
				maxInfectedNeighboursCount = infectedNeighboursCount;
				targetCity = city;
			}
		}
		if(targetCity!=null) {
			if(Client.alredyInDistanceMap(distanceMap, targetCity.getName())) {
				cityToGo = Client.getDirectionToMove(b.getPawnsLocations(playerID), targetCity.getName(), distanceMap, b);	
				movetoexecute = checkDTmoves(posMoves,cityToGo,playerID);
				if(movetoexecute.equals("notAMove")) {
					movetoexecute = checkFlyMove(posMoves,cityToGo,playerID);
					if(!movetoexecute.equals("notAMove"))
						return movetoexecute;
				}
			}
		...
		if(movetoexecute.equals("notAMove"))
			movetoexecute = getPromisingTDMoves(b,posMoves,playerID);
			
		
		return movetoexecute;
	}

public ArrayList<City> getCitiesOfColour(String color){
	ArrayList<City> cc = new ArrayList<City>();
...
public String opsExpertMove(ArrayList<String> r, Board b, int playerID) {
	
	ArrayList<citiesWithDistancesObj> distanceMap = new ArrayList<citiesWithDistancesObj>();
	String currLocation = b.getPawnsLocations(playerID);
	distanceMap = Client.buildDistanceMap(b, currLocation, distanceMap);
	String goalCity = currLocation;
	String DTtoGO;
	String movetoexecute = "notAMove";
	City cityToBRS = null;
	int neighboursNumber = 0;
	ArrayList<City> citiesofc = null;
	boolean mustBuild[] = {true, true, false, true};
	
	citiesofc = getCitiesOfColour(searchForCity(currLocation).getColour());
	
	
	if(!searchForCity(currLocation).getColour().equals("Blue")) {
		for(City city : citiesofc) {
			if(city.getHasReseachStation()) {
				for(int i=0;i<4;i++) {
					if(colors[i].equals(searchForCity(currLocation).getColour())) {
						mustBuild[i] = false;
						break;	
					}						
				}
		...
		for(int i=0;i<4;i++) {
			if(mustBuild[i] && colors[i].equals(searchForCity(currLocation).getColour())) {
				for(City city : citiesofc) {
					if(city.getNeighboursNumber()>neighboursNumber) {
						neighboursNumber=city.getNeighboursNumber();
						cityToBRS = city;					
					}
				}
				break;
	...
	}else if(searchForCity(currLocation).getColour().equals("Blue") && b.getResearchStationsBuild()>3) {
		if((b.searchForCity("Madrid")).getHasReseachStation() || (b.searchForCity("Paris")).getHasReseachStation())
			movetoexecute = "notAMove";
		else if(!(b.searchForCity("Madrid")).getHasReseachStation())
			cityToBRS = b.searchForCity("Madrid");
		else if(!(b.searchForCity("Paris")).getHasReseachStation())
			cityToBRS = b.searchForCity("Madrid");
	}
	if(cityToBRS!=null) {
		if(!b.getRSLocations().contains(cityToBRS.getName())) {
			if(cityToBRS.getName().equals(currLocation))
				movetoexecute = "#BRS,"+playerID+","+cityToBRS.getName();
			else if(b.getHandOf(playerID).contains(cityToBRS.getName()))
				return "#DF,"+playerID+","+cityToBRS.getName();
			else if(b.getHandOf(playerID).contains(currLocation))
				return "#CF,"+playerID+","+currLocation;
			else {
				DTtoGO=Client.getDirectionToMove(currLocation, cityToBRS.getName(), distanceMap, b);
				for(String move : r) {
					if(move.contains("#DT,"+playerID+","+DTtoGO))
						movetoexecute = move;
					}		
			}
	}else
		movetoexecute = getPromisingTDMoves(b,r,playerID);
	}else {
		movetoexecute = getPromisingTDMoves(b,r,playerID);
	}
	return movetoexecute;
	...
	public String checkForCureMove(ArrayList<String> r,Board theBoard,int thePlayerID) {
		
    	ArrayList<citiesWithDistancesObj> distanceMap = new ArrayList<citiesWithDistancesObj>();
		String currLocation = theBoard.getPawnsLocations(thePlayerID);
    	distanceMap = Client.buildDistanceMap(theBoard, currLocation, distanceMap);
		int minDist = -1;
		String goalCity = currLocation;
		String DTtoGO;
		...
		if(theBoard.getRoleOf(thePlayerID).equals("Scientist")) {
			if(Client.cardsCounterOfColor(theBoard, thePlayerID,"Black")==3) {
				
				for(String rs : theBoard.getRSLocations()) {
					//DT move
					if(Client.isInDistanceMap(distanceMap, rs)) {
						int tryDist = Client.distanceFrom(rs, distanceMap);
						if(tryDist==1) 
							return "#DT,"+thePlayerID+","+rs;
					...
					//DF move
					if(theBoard.getHandOf(thePlayerID).contains(rs) && !searchForCity(rs).getColour().equals("Black"))
						return "#DF,"+thePlayerID+","+rs;
					
					//CF move
					if(theBoard.getHandOf(thePlayerID).contains(currLocation) && !searchForCity(currLocation).getColour().equals("Black")) 
						return "#CF,"+thePlayerID+","+rs;
					
				}
				
				//here we have the RS with the minimum distance
				//from where we are
				DTtoGO=Client.getDirectionToMove(currLocation, goalCity, distanceMap, theBoard);
				for(String move : r) {
					if(move.contains("#DT,"+thePlayerID+","+DTtoGO))
						return move;
				}
				
				//here we come when all the RS are not
				//in the distance map and we can't DF,CF
				
			}else if (Client.cardsCounterOfColor(theBoard, thePlayerID,"Black")>3) {
				for(String rs : theBoard.getRSLocations()) {
					//DT move
					if(Client.isInDistanceMap(distanceMap, rs)) {
						int tryDist = Client.distanceFrom(rs, distanceMap);
					...
					//DF move
					if(theBoard.getHandOf(thePlayerID).contains(rs))
						return "#DF,"+thePlayerID+","+rs;
					
					//CF move
					if(theBoard.getHandOf(thePlayerID).contains(currLocation)) 
						return "#CF,"+thePlayerID+","+rs;
			...
			if(Client.cardsCounterOfColor(theBoard, thePlayerID,"Red")>3) {
				for(String rs : theBoard.getRSLocations()) {
					//DT move
					if(Client.isInDistanceMap(distanceMap, rs)) {
						int tryDist = Client.distanceFrom(rs, distanceMap);
			...
			}else if(Client.cardsCounterOfColor(theBoard, thePlayerID,"Red")==3) {
				for(String rs : theBoard.getRSLocations()) {
					//DT move
					if(Client.isInDistanceMap(distanceMap, rs)) {
						int tryDist = Client.distanceFrom(rs, distanceMap);
					...
					//DF move
					if(theBoard.getHandOf(thePlayerID).contains(rs) && !searchForCity(rs).getColour().equals("Red"))
						return "#DF,"+thePlayerID+","+rs;
					
					//CF move
					if(theBoard.getHandOf(thePlayerID).contains(currLocation) && !searchForCity(currLocation).getColour().equals("Red")) 
						return "#CF,"+thePlayerID+","+rs;
			...
			if(Client.cardsCounterOfColor(theBoard, thePlayerID,"Yellow")==3) {
				for(String rs : theBoard.getRSLocations()) {
					//DT move
					if(Client.isInDistanceMap(distanceMap, rs)) {
						int tryDist = Client.distanceFrom(rs, distanceMap);
					...
					//DF move
					if(theBoard.getHandOf(thePlayerID).contains(rs) && !searchForCity(rs).getColour().equals("Yellow"))
						return "#DF,"+thePlayerID+","+rs;
					
					//CF move
					if(theBoard.getHandOf(thePlayerID).contains(currLocation) && !searchForCity(currLocation).getColour().equals("Yellow")) 
						return "#CF,"+thePlayerID+","+rs;
			...
			}else if(Client.cardsCounterOfColor(theBoard, thePlayerID,"Yellow")>3) {
				for(String rs : theBoard.getRSLocations()) {
					//DT move
					if(Client.isInDistanceMap(distanceMap, rs)) {
						int tryDist = Client.distanceFrom(rs, distanceMap);
			...
			if(Client.cardsCounterOfColor(theBoard, thePlayerID,"Blue")==3) {
				for(String rs : theBoard.getRSLocations()) {
					//DT move
					if(Client.isInDistanceMap(distanceMap, rs)) {
						int tryDist = Client.distanceFrom(rs, distanceMap);
					...
					//DF move
					if(theBoard.getHandOf(thePlayerID).contains(rs) && !searchForCity(rs).getColour().equals("Blue"))
						return "#DF,"+thePlayerID+","+rs;
					
					//CF move
					if(theBoard.getHandOf(thePlayerID).contains(currLocation) && !searchForCity(currLocation).getColour().equals("Blue")) 
						return "#CF,"+thePlayerID+","+rs;
			...
			}else if(Client.cardsCounterOfColor(theBoard, thePlayerID,"Blue")>3) {
				for(String rs : theBoard.getRSLocations()) {
					//DT move
					if(Client.isInDistanceMap(distanceMap, rs)) {
						int tryDist = Client.distanceFrom(rs, distanceMap);
		...
		}else {//here we check if we have 4 same color cards
			if(Client.cardsCounterOfColor(theBoard, thePlayerID,"Black")==4) {
			...
			}else if (Client.cardsCounterOfColor(theBoard, thePlayerID,"Black")>4) {
				for(String rs : theBoard.getRSLocations()) {
					//DT move
					if(Client.isInDistanceMap(distanceMap, rs)) {
						int tryDist = Client.distanceFrom(rs, distanceMap);
			...
			if(Client.cardsCounterOfColor(theBoard, thePlayerID,"Red")>4) {
				for(String rs : theBoard.getRSLocations()) {
					//DT move
					if(Client.isInDistanceMap(distanceMap, rs)) {
						int tryDist = Client.distanceFrom(rs, distanceMap);
			...
			}else if(Client.cardsCounterOfColor(theBoard, thePlayerID,"Red")==4) {
				for(String rs : theBoard.getRSLocations()) {
					//DT move
					if(Client.isInDistanceMap(distanceMap, rs)) {
						int tryDist = Client.distanceFrom(rs, distanceMap);
			...
			if(Client.cardsCounterOfColor(theBoard, thePlayerID,"Yellow")==4) {
				for(String rs : theBoard.getRSLocations()) {
					//DT move
					if(Client.isInDistanceMap(distanceMap, rs)) {
						int tryDist = Client.distanceFrom(rs, distanceMap);
			...
			}else if(Client.cardsCounterOfColor(theBoard, thePlayerID,"Yellow")>4) {
				for(String rs : theBoard.getRSLocations()) {
					//DT move
					if(Client.isInDistanceMap(distanceMap, rs)) {
						int tryDist = Client.distanceFrom(rs, distanceMap);
			...
			if(Client.cardsCounterOfColor(theBoard, thePlayerID,"Blue")==4) {
				for(String rs : theBoard.getRSLocations()) {
					//DT move
					if(Client.isInDistanceMap(distanceMap, rs)) {
						int tryDist = Client.distanceFrom(rs, distanceMap);
			...
			}else if(Client.cardsCounterOfColor(theBoard, thePlayerID,"Blue")>4) {
				for(String rs : theBoard.getRSLocations()) {
					//DT move
					if(Client.isInDistanceMap(distanceMap, rs)) {
						int tryDist = Client.distanceFrom(rs, distanceMap);
	...
	public static Board performMove(String move,Board b){
		
		String delimiterVariables = ",";
		String[] actions;
		Board cpBoard = Client.copyBoard(b);
		//split the action at commas
		actions = move.split(delimiterVariables);
		
		if(actions[0].equals("#DT")) {//drive ferry
			if(cpBoard.driveTo(Integer.parseInt(actions[1]),actions[2])) {
				
				return cpBoard;
			}
			//action invalid
			//return null;
		}else if(actions[0].equals("#DF")) {//direct flight
			if(cpBoard.directFlight(Integer.parseInt(actions[1]),actions[2])) {
				return cpBoard;
			}
			//return null;
		}else if(actions[0].equals("#CF")) {//charter flight
			if(cpBoard.charterFlight(Integer.parseInt(actions[1]),actions[2])) {
				return cpBoard;
			}
			//return null;
		}else if(actions[0].equals("#SF")) {//shuttle flight
			if(cpBoard.shuttleFlight(Integer.parseInt(actions[1]),actions[2])) {
				return cpBoard;
			}
			//return null;
		}else if(actions[0].equals("#BRS")) {//build RS
			if(cpBoard.buildRS(Integer.parseInt(actions[1]),actions[2])) {
				return cpBoard;
			}
			//return null;
		}else if(actions[0].equals("#RRS")) {//remove RS,HAVETOFIX POTENTIALMOVES
			if(cpBoard.removeRS(Integer.parseInt(actions[1]),actions[2])) {
				return cpBoard;
			}
			//return null;
		}else if(actions[0].equals("#OET")) {
			cpBoard.operationsExpertTravel(Integer.parseInt(actions[1]), actions[2], actions[3]);
			return cpBoard;
			
			
		}else if(actions[0].equals("#TD")) {//treat disease
			if(cpBoard.treatDisease(Integer.parseInt(actions[1]),actions[2],actions[3])) {
				return cpBoard;
			}
			//return null;
		}else if(actions[0].equals("#CD1")) {//cure disease1
			if(cpBoard.cureDisease(Integer.parseInt(actions[1]),actions[2])) {
				return cpBoard;
			}
			//return null;
		}else if(actions[0].equals("#CD2")) {//cure disease2
			if(cpBoard.getRoleOf(Integer.parseInt(actions[1])).equals("Scientist")) {
				if(cpBoard.cureDisease(Integer.parseInt(actions[1]),actions[2],actions[3],actions[4],actions[5],null)) {
					return cpBoard;
				}
				//return null;
			}
			//not scientist
			if(cpBoard.cureDisease(Integer.parseInt(actions[1]),actions[2],actions[3],actions[4],actions[5],actions[6])) {
				return cpBoard;
			}
			//return null;
		}
	...
	// --> Getters & Setters <--
	public ArrayList<String> getOtherPlayers(int playerID){
		
		ArrayList<String> otherS = new ArrayList<String>();
		String delim = "_";
		for(int i=0;i<usernames.length;i++) {
			if(!usernames[i].equals("User_"+playerID)) {
				otherS.add(usernames[i].split(delim)[1]);
			}
		}
